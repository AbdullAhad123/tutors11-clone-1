        ...
        switch($qType) {
            case "MTF":
                ...
                foreach ($leftOptions as $key => $option) {
                    array_push($matches, ['id' => md5(config('tutor11.matching_option_salt').$key), 'value' => $option]);
                }
                foreach ($rightPairs as $key => $pair) {
                    array_push($pairs, ['id' => md5(config('tutor11.matching_pair_salt').$key), 'value' => $pair, 'code' => '']);
                }
                shuffle($pairs);
            ...
            case "ORD":
                foreach ($options as $key => $option) {
                    array_push($newOptions, ['id' => md5(config('tutor11.ordering_option_salt').$key), 'value' => $option['option'], 'code' => '']);
                }
                shuffle($newOptions);
...
if(!function_exists('validateMTF')) {
    ...
    function validateMTF($options, $userResponse, $answerFlag = false) {
        ...
        $responseMatch = [];
        foreach ($options as $key => $option) {
            array_push($correctMatch, md5(config('tutor11.matching_pair_salt').$key));
        }
...
if(!function_exists('validateSAQ')) {
    ...
    function validateSAQ($options, $userResponse) {
        ...
        $possibleAnswers = [];
        foreach ($options as $option) {
            array_push($possibleAnswers, strtolower($option['option']));
        }
...
if(!function_exists('validateORD')) {
    ...
    function validateORD($options, $userResponse, $answerFlag = false) {
        ...
        $responseOrder = [];
        foreach ($options as $key => $option) {
            array_push($correctOrder, md5(config('tutor11.ordering_option_salt').$key));
        }
        foreach ($userResponse as $item) {
            if(is_array($item)) {
