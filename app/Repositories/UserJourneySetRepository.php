<?php
/**
 * File name: UserJourneySetRepository.php
...
class UserJourneySetRepository
{
    /**
     * Get existing journey session of a user, if any
     *
     * @param JourneySet $set
     * @return \Illuminate\Database\Eloquent\Model|\Illuminate\Database\Eloquent\Relations\HasMany|object|null
     */
    public function getSession(JourneySet $set)
    {
        return $set->sessions()->where('user_id', auth()->user()->id)->latest()->first();
    }

    /**
     * Create a new journey session for an user
     *
     * @param JourneySet $set
     * @return |null
     */
    public function createSession(JourneySet $set)
    {
        $session = JourneySession::create([
            'journey_set_id' => $set->id,
    ...
    /**
     * Generate user journey session analytics
     *
     * @param $session
     * @param $total
     * @return array
