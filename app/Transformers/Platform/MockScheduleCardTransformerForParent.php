...
class MockScheduleCardTransformerForParent extends TransformerAbstract
{
    /**
    ...
    public function transform(mock_Schedules $schedule)
    {
        ...
        if(count(SuggestedMockTest::where("user_id",auth()->user()->roles[0]->name!="parent"?auth()->user()->id:session("selected_child")["id"])->where("mock_test_id",$schedule->mock->id)->get()) > 0 ){
