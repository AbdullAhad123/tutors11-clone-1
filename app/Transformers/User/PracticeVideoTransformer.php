...
class PracticeVideoTransformer extends TransformerAbstract
{
    ...
    public function transform(Video $video)
    {
        return [
            ...
            'paid' => $video->is_paid,
            'subscription' => $video->is_paid && !$this->subscription ? false : true,
            'sort_order' => $video->pivot->sort_order,
            'created_at' => Carbon::parse($video->created_at)->format('M d, Y'),
            'difficulty' => $video->difficulty_level_id ? DifficultyLevel::findOrFail($video->difficulty_level_id)->code : 'VERYEASY',
        ];
    }
}
