...
class ExamQuestionController extends Controller
{
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function fetchAvailableQuestions($id, $section, QuestionFilters $filters)
    {
        $examsection = ExamSection::select(['section_id'])->findOrFail($section);
        $skill = Skill::where('section_id','=',$examsection->section_id)->pluck('id');
        
        $exam = Exam::select(['id', 'title'])->with(['questions' => function ($builder) {
            $builder->select('id');
        }])->findOrFail($id);
    ...
    public function addQuestion(Exam $exam, ExamSection $section)
    {
        try {
            ...
            $attach = $section->questions()->attach([
                $question->id => ['exam_id' => $exam->id]
            ]);
            
            $contains = $section->questions->contains($question->id);
    ...
    public function randomlyAddQuestions(Exam $exam, Request $request)
    {
        $added = [];
        foreach ($request->set as $i => $set) {
            $section = ExamSection::where('id', $set['section_id'])->first();
            $skillIds = Skill::where('section_id', $section->section_id)->pluck('id');
            
            for ($i = 1; $i <= $set['limit']; $i++) {
                $question = Question::select('id', 'skill_id')
                    ->whereHas('skill')
                ...
                if ($question) {
                    if (!$section->questions()->where('question_id', $question->id)->exists()) {
                        $section->questions()->attach([
                            $question->id => ['exam_id' => $exam->id]
                        ]);
                        array_push($added, $question->id);
                    }
                }
            }
        ...
        $exam->updateMeta();
        return redirect()->route('exams.schedules.index', ['exam' => $exam->id]);
    }
