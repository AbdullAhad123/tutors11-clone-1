...
class ExamController extends Controller
{
    ...
    public function __construct(UserExamRepository $repository, QuestionRepository $questionRepository,ColorsRepository $colors)
    {
    ...
    public function instructions($slug)
    {
        ...
        $exam_schedule = DB::table('exam_schedules')->where('exam_id', $exam->id)->where('status', '=', "active")->first();
        $now = Carbon::now()->toDateTimeString();
        return view('User/ExamInstructions', [ 
    ...
    {
        $subscription = request()->user()->hasActiveSubscription(request()->user()->id);
        $loadCount = $exam->loadCount(['sessions' => function ($query) {
            $query->where('user_id', auth()->user()->id)->where('status', 'completed');
        }]);
        if($loadCount->sessions_count > 0){
            $session = ExamSession::where('exam_id', $exam->id)->where('user_id', auth()->user()->id)->first();
    ...
    public function goToExam(Exam $exam, $session)
    {
        ...
        $answeredQuestions = 0;
        $sections = fractal($session->sections, new ExamSessionSectionTransformer())->toArray()['data'];
        foreach ($session->sections as $key => $section) {
            $answeredQuestions += $session->questions()->wherePivot('exam_section_id', $section->id)->wherePivotIn('status', ['answered', 'not_answered'])->count();
            $sections[$key]['questions'] = fractal(
                $session->questions()->with(['questionType:id,name,code', 'skill:id,name', 'comprehensionPassage:id,body'])
                    ...
                    ->orderBy('sno')
                    ->get(),
                new ExamQuestionTransformer()
            )->toArray()['data'];
            $sections[$key]['answered'] = $session->questions()->wherePivot('exam_section_id', $section->id)->wherePivotIn('status', ['answered', 'not_answered'])->count();
        }
        return view('User/ExamScreen', [
    ...
    public function getExamSectionQuestions(Exam $exam, $session, $section)
    {
        ...
        return response()->json([
            ...
            'answered_mark_for_review' => $session->questions()->wherePivot('exam_section_id', $section)->wherePivotIn('status', ['answered_mark_for_review'])->count(),
            'mark_for_review' => $session->questions()->wherePivot('exam_section_id', $section)->wherePivotIn('status', ['mark_for_review'])->count(),
            'total_questions' => $session->questions()->wherePivot('exam_section_id', $section)->count(),
            'remainingTime' => $remainingTime
        ], 200);
    }
...
// dd($session);
    ...
    public function thankYou(Exam $exam, $session)
    {
        ...
        // dd($session); 
        return view('User/ExamThanksScreen', [
    ...
    public function results(Exam $exam, $session)
    {
        ...
        return view('User/ExamResults/Analysis', [
            ...
            'steps' => $this->repository->getExamProgressLinks($exam->slug, $session->code, 'exam_results'),
            "childName" => $childName
        ]);
    }
    ...
    public function solutions(Exam $exam, $session)
    {
        ...
        foreach($sections as $section) {
            ...
            if($section['results']){
                $correct_answered_questions = $correct_answered_questions + $section['results']['correct_answered_questions'];
                $accuracy = $accuracy + $section['results']['accuracy'];
                $total_time_taken = $total_time_taken + $section['results']['total_time_taken']['seconds'];
            }
            $questions = fractal(
                $session->questions()->wherePivot('exam_section_id', $section['id'])->with(['questionType:id,name,code', 'skill:id,name'])
            ...
            array_push($sections_questions,$sections_question);
        }
        $total_section_accuracy = ($accuracy / $total_accuracy) * 100;
        return view('User/ExamResults/Solutions', [
            'exam' => $exam->only('code', 'title', 'slug', 'total_questions', 'total_marks', 'settings'),
            "chartColors"=>$this->chart_colors,
            'sections' => $sections,
            'session' => $session->only('code', 'total_time_taken', 'results', 'status'),
            'steps' => $this->repository->getExamProgressLinks($exam->slug, $session->code, 'exam_solutions'),
            "childName" => $childName,
            'sections_questions' => $sections_questions,
            'correct_answered_questions' => $correct_answered_questions,
            'total_section_accuracy' => round($total_section_accuracy),
    ...
    public function fetchSolutions(Exam $exam, $session, $section)
    {
        ...
        // dd($session);
        $questions = fractal(
            $session->questions()->wherePivot('exam_section_id', $section)->with(['questionType:id,name,code', 'skill:id,name'])
                ->orderBy('sno')->get(['id', 'code', 'question', 'question_type_id', 'skill_id', 'solution', 'solution_video']),
            new QuizSolutionTransformer()
        )->toArray();
    ...
    public function leaderboard(Exam $exam, $session)
    {
        ...
        return view('User/ExamResults/LeaderBoard', [
            'exam' => $exam->only('code', 'title', 'slug', 'total_questions', 'total_marks', 'settings'),
            'session' => $session->only('code', 'total_time_taken', 'results', 'status', 'user'),
