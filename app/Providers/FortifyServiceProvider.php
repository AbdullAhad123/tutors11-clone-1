...
class FortifyServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Fortify::authenticateUsing(function (Request $request) {
            ...
            if (($user && Hash::check($request->password, $user->password)) || (session()->has("selected_child") && session("selected_child")["email"] == $request->email)) {
                if(!config('tutor11.demo_mode')) {
                    //Delete Other Browser Sessions on Login
                    $guard = app(StatefulGuard::class);
                    $guard->logoutOtherDevices($request->password);
                    if (config('session.driIlluminate\Contracts\Foundation\Application::logoutOtherDevices()ver') == 'database') {
