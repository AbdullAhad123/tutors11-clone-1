...
<head>
    ...
    <style>
        ...
        #user_Dropdown {max-height: 200px;}
    </style>
</head>
...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Subscriptions</h1>
            <p class="text-dark my-2 fs-5">Admins and Instructors can add, manage and update subscriptions details effortleesly.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow my-4">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Manage Subscriptions</h2>
                <button  data-bs-toggle="modal" data-bs-target="#newSubscriptionModal" class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_160">CODE</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_200">PLAN</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_180">USER</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_200">STARTS</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_200">ENDS</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_300">PAYMENT</th>
                    ...
                    <tbody>
                        <tr class="row_bg_light">
                            ...
                            <td class="py-4" data-label="search">
                                <select class="form-select select_custom" id="searchPlan">
                                    <option selected>Search Plan</option>
                            ...
                            <td class="py-4" data-label="search"></td>
                            <td class="py-4" data-label="search">
                                <input type="text" id="searchPayment" class="shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Payment">
                            </td>
                            <td class="py-4" data-label="search">
                                <select class="form-select select_custom" is="searchStatus">
                                ...
                                <tr>
                                    <td class="py-4" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$subscription['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4" data-label="exam">{{$subscription['plan']}}</td>
                                    <td class="py-4" data-label="completed">{{$subscription['user']}}</td>
                                    <td class="py-4" data-label="completed">{{$subscription['starts']}}</td>
                                    <td class="py-4" data-label="completed">{{$subscription['ends']}}</td>
                                    <td class="py-4" data-label="completed">{{$subscription['payment']}}</td>
                                    ...
                                    <td class="py-4" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/subscriptions/{{$subscription['id']}}/details">Details</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item cursor-pointer delete" data-id="{{$subscription['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg-label-primary px-1">
                                @if($subscriptions()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($subscriptions()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($subscriptions()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($subscriptions()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($subscriptions()['meta']['pagination']['total'] > $subscriptions()['meta']['pagination']['count'])
                            @isset($subscriptions()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$subscriptions()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$subscriptions()['meta']['pagination']['current_page']}}</span> OF {{$subscriptions()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($subscriptions()['meta']['pagination']['total'] > $subscriptions()['meta']['pagination']['count'])
                            @isset($subscriptions()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$subscriptions()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
        ...
        <!-- Create New Manual Subscription Modal -->
        <div class="modal fade" id="newSubscriptionModal" aria-labelledby="modalToggleLabel" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-dark" id="modalToggleLabel">New Manual Subscription</h5>
                ...
                <div class="modal-footer">
                    <div class="w-100 text-end">
                        <div class="mb-1 font-size-13 text-danger" id="NewsubscriptionFormError"></div>
                        <button class="btn btn-primary" id="createNewsubscription">Create</button>
                    </div>
                </div>
                </div>
            </div>
    ...
    <script>
        $('#searchCode, #searchPayment').on('keyup', function(e) {
            var code = e.keyCode || e.which;
            if (code == 13) {
                let queryCode = $("#searchCode").val();
                let queryPayment = $("#searchPayment").val();
                let testCode = queryCode ? "&code=" + queryCode : "";
                let testPayment = queryPayment ? "&payment=" + queryPayment : "";
                let url = "/admin/subscriptions?" + testCode + testPayment;
                window.location.href = url;
            }
        });
        // plan change
        $("#searchPlan").on('change', function() {
            let selectPlan = $(this).val();
            if (selectPlan != "Search Plan") {
                let testPlan = "&plan=" + selectPlan;
                let queryCode = $("#searchCode").val();
                let queryPayment = $("#searchPayment").val();
                let testCode = queryCode ? "&code=" + queryCode : "";
                let testPayment = queryPayment ? "&payment=" + queryPayment : "";
                let url = "/admin/subscriptions?" + testCode + testPayment + testPlan;
                window.location.href = url;
            }
        }) 
        // status change
        $("#searchStatus").on('change', function() {
            ...
            if (selectStatus != "Search Status") {
                ...
                let testCode = queryCode ? "&code=" + queryCode : "";
                let testPayment = queryPayment ? "&payment=" + queryPayment : "";
                let url = "/admin/subscriptions?" + testCode + testPayment + testStatus;
                window.location.href = url;
            }
        });
        ...
        $('#per_page_option').on('change', function(e) {
            ...
            var current_page = {!!str_replace("'", "\'", json_encode($subscriptions()['meta']['pagination']['current_page'])) !!};
            if (perPage == 10) {
        ...
        $("#createNewsubscription").click(function(){
            ...
            if(user_id){
                if(plan_id){
                    ...
                    $("#NewsubscriptionFormError").text("Please select plan from plan dropdown");
                }
            } else {
                $("#NewsubscriptionFormError").text("Please select user from user dropdown");
            }
        });

        $("#user_id, #plan_id, #status").on('change', function() {
            $("#NewsubscriptionFormError").text();
        });
