...
<body>
    ...
    <section class="p-4">
        <div class="my-4">
            <h1 class="text-dark my-2">Quiz Types</h1>
            <p class="text-dark fs-5 my-3">Efficiently oversee various quiz types such as assignments, contests, hackathons, daily tasks, and more on the admin dashboard for seamless management.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Quiz Types</h2>
                <a href="/admin/quiz-types/create"><button class="btn rounded-2 text-uppercase create_new_btn text-white "><i class='bx bx-plus-circle mb-1 text-white'></i>Add new</button></a>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold w_160 fs-6 text-dark py-3">CODE</th>
                        <th scope="col" class="font_bold w_200 fs-6 text-dark py-3">NAME</th>
                        <th scope="col" class="font_bold w_160 fs-6 text-dark py-3">STATUS</th>
                        <th scope="col" class="font_bold w_160 fs-6 text-dark py-3">ACTIONS</th>
                    </thead>
                    <tbody>
                        <tr class="row_bg_light">
                            <td class="py-4" data-label="search">
                                <input type="text" data-name="code" id="searchCode" class="shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Code">
                            </td>
                            <td class="py-4" data-label="search">
                                <input type="text" data-name="name" id="searchTitle" class="shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Title">
                            </td>
                            <td class="py-4" data-label="search">
                        ...
                        @if($quizTypes()['meta']['pagination']['count'] > 0)
                            @foreach($quizTypes()['data'] as $quizType)
                                <tr>
                                    <td class="py-4" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$quizType['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4" data-label="exam">{{$quizType['name']}}</td>
                                    ...
                                    <td class="py-4" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/quiz-types/{{$quizType['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item deleteQuizType cursor-pointer" data-id="{{$quizType['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($quizTypes()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($quizTypes()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($quizTypes()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($quizTypes()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($quizTypes()['meta']['pagination']['total'] > $quizTypes()['meta']['pagination']['count'])
                            @isset($quizTypes()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$quizTypes()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$quizTypes()['meta']['pagination']['current_page']}}</span> OF {{$quizTypes()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($quizTypes()['meta']['pagination']['total'] > $quizTypes()['meta']['pagination']['count'])
                            @isset($quizTypes()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$quizTypes()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
        ...
        $(document).ready(function() {
            ...
            $('#searchCode, #searchTitle').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/quiz-types?" + testCode + testName;
            ...
            $("#select_status").change(function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testName = queryName ? "&name=" + queryName : "";
                    let url = "/admin/quiz-types?" + testCode + testName + testStatus;
                    window.location.href = url;
                }
            })
            // row per page 
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($quizTypes()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
