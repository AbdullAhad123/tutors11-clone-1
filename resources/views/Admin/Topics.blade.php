...
<head>
    ...
    <title>Sub-topics - Manage Subjects - 
...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Sub Topics</h1>
            <p class="text-dark fs-5 my-2">Admins and instructors can effortlessly manage, add, and update sub-topics for any topic of any subject with just a click.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow my-4">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Manage Sub-topics</h2>
                <button data-bs-toggle="modal" data-bs-target="#newTopicModal" class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button>
            </div>
            ...
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        ...
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_300">TOPIC</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_160">YEAR</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_160">TOTAL Q's</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_160">HELPSHEET</th>
                    ...
                    <tbody>
                        <tr class="row_bg_light">
                            <td class="py-4 text-dark" data-label="search">
                                <input type="text" id="searchCode" data-url="code" class="search shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Code">
                            </td>
                            <td class="py-4 text-dark" data-label="search">
                                <input type="text" id="searchTitle" data-url="name" class="search shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Title">
                            </td>
                            <td class="py-4 text-dark" data-label="search">
                                <input type="text" id="searchTopic" data-url="topic" class="search shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Topic">
                            </td>
                            <td class="py-4 text-dark" data-label="search">
                                <input type="text" id="searchYear" data-url="year" class="search shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Year">
                            </td>
                            <td class="py-4 text-dark" data-label="search"></td>
                            <td class="py-4 text-dark" data-label="search"></td>
                            <td class="py-4 text-dark" data-label="search">
                                ...
                                <tr>
                                    <td class="py-4 text-dark" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$topic['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4 text-dark" data-label="exam">{{$topic['name']}}</td>
                                    <td class="py-4 text-dark" data-label="completed">{{$topic['skill']}}</td>
                                    <td class="py-4 text-dark" data-label="completed" class="text-nowrap">{{$topic['year']}}</td>
                                    <td class="py-4 text-dark" data-label="completed">{{$topic['ques_count']}} Q's</td>
                                    <td class="py-4 text-dark" data-label="completed">
                                        @if($topic['helpsheet'] != '--')
                                            <a href="{{url($topic['helpsheet'])}}" target="_blank"><img class="rounded" src="{{url($topic['helpsheet'])}}" alt="helpsheet" width="50"></a>
                                        @else
                                            --
                                        @endif
                                    </td>
                                    <td class="py-4 text-dark" data-label="status">
                                        @if($topic['status'])
                                        <span class="pp-2 px-3 rounded small bg_label_published">Active</span>
                                        @else
                                        <span class="pp-2 px-3 rounded small bg_label_draft">In-Active</span>
                                        @endif
                                    </td>
                                    <td class="py-4 text-dark" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/topics/{{$topic['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item cursor-pointer delete" data-id="{{$topic['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($topics()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($topics()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($topics()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($topics()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($topics()['meta']['pagination']['total'] > $topics()['meta']['pagination']['count'])
                                @isset($topics()['meta']['pagination']['links']['previous'])
                                    <span>
                            ...
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$topics()['meta']['pagination']['current_page']}}</span> OF {{$topics()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($topics()['meta']['pagination']['total'] > $topics()['meta']['pagination']['count'])
                                 @isset($topics()['meta']['pagination']['links']['next'])
                                    <span>
                        ...
                        <div class="modal-body">
                            <div class="form-input">
                                <div class="my-4">
                                    ...
                                    <input type="text" name="name" class="form-control mt-3" id="newTopicName" required>
                                    <span id="newTopicName_error" class="text-danger small d-none"></span>
                                </div>

                                <div class="my-4">
                                    <label>Select Section</label>
                                    <select class="form-select mt-3 bg-transparent" id="selectSection">
                                ...
                                <div class="my-4">
                                    <label>Topics</label>
                                    <select name="skill_id" class="form-select mt-3 bg-transparent" id="selectTopic" required>
                                        @foreach($skills as $skill)
                                            <option value="{{$skill['id']}}" data-section_id="{{$skill['section_id']}}" class="d-none">{{$skill['name']}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="my-4">
                                    <label>Help Sheet</label>
                                    <input type="file" name="helpsheet_photo" id="helpsheet_photo" class="mt-3 form-control">
                                </div>

                                <div class="my-4">
                                    <label>Description</label>
                                    <textarea name="short_description" cols="40" rows="2" class="text_area_div my-3 form-control"
                                    ...
                                    <div class="row m-0 align-items-center justify-content-between">
                                        ...
                                        <div class="col-2 ">
                                            <div class="form-check form-switch text-center">
                                                <input name="is_active" class="form-check-input shadow-none border border-dark btn-lg"
    ...
    <script>
        ...
        function getTopics(id){
            $("#selectTopic").find("option").addClass("d-none");
            $("#selectTopic").find("option[data-section_id="+id+"]").removeClass("d-none");
        }
        $(document).ready(function(){
            // on keyup search 
            var current_page = {!!str_replace("'", "\'", json_encode($topics()['meta']['pagination']['current_page'])) !!};
            ...
            $("#searchStatus").on('change', function() {
                ...
                if (selectStatus != "Search Status") {
                    filterSearch("status",selectStatus);
                }
            })
            // copy code 
