...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Regions</h1>
            <p class="text-dark fs-5 my-2">Simplify school management with our schools Hub. Admins and instructors can effortlessly add, update, and manage the list of regions on the website, ensuring accurate and relevant information.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Manage Regions</h2>
                <a href="{{route('region_create')}}"><button class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button></a>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_300">NAME</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_180">SLUG</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_200">DESCRIPTION</th>
                    ...
                    <tbody>
                        ...
                        @if(count($regions()['data']) > 0)
                            @foreach($regions()['data'] as $region)
                                <tr>
                                    <td class="py-4 text-dark">{{$region['name']}}</td>
                                    <td class="py-4 text-dark">{{$region['slug']}}</td>
                                    <td class="py-4 text-dark">{{$region['description']}}</td>
                                    <td class="py-4 text-dark">{{$region['created_at']}}</td>
                                    <td class="py-4 text-dark">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="{{route('region_edit',['id'=>$region['id']])}}">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item cursor-pointer delete" data-url="{{route('region_delete',['id'=>$region['id']])}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                        ...
                        @else
                            <tr>
                                <td colspan="5" class="py-4">
                                    <h2 class="text-dark my-2 h5 text-center">
                                        NO RECORDS FOUND
                                    </h5>
                                </td>
                            </tr>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg-label-primary px-1">
                                @if($regions()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($regions()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($regions()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($regions()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($regions()['meta']['pagination']['total'] > $regions()['meta']['pagination']['count'])
                            @isset($regions()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$regions()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$regions()['meta']['pagination']['current_page']}}</span> OF {{$regions()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($regions()['meta']['pagination']['total'] > $regions()['meta']['pagination']['count'])
                            @isset($regions()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$regions()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($regions()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
