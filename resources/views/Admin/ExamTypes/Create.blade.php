    ...
    <section class="p-4">
        ...
        <div class="bg-white p-4 rounded-3 shadow">
            <div class="form-group mb-4">
                <label for="name" class="text-dark fw-medium">Exam Type Name</label>
                <input type="text" class="form-control shadow-sm mt-2 border" id="name" placeholder="Enter Name..." value="@if($editFlag){{$ExamType['name']}}@endif">
            </div>
            <div class="form-group mb-4">
                <label for="color" class="text-dark fw-medium">Color</label>
                <div class="d-flex align-items-lg-center">
                    <input class="me-2 p-0 border-0 overflow-hidden rounded cursor-pointer" type="color" id="color" name="color" @if($editFlag) value="#{{$ExamType['color']}}" @else value="#444444" @endif>
                    <input type="text" class="form-control shadow-sm border" id="colorText" value="@if($editFlag){{$ExamType['color']}}@endif">
                </div>
            </div>
            <div class="form-group mb-4">
                <label for="summernote" class="text-dark fw-medium">Description</label>
                <div class="my-3">
                    <textarea id="summernote">@if($editFlag){{$ExamType['description']}}@endif</textarea>
                </div>
            </div>
            ...
            <div class="mb-4 row">
                <div class="col-12 col-sm-8">
                    <div id="allowActiveText" class="h6 mb-1 text-dark">
                        @if($editFlag && $ExamType['is_active'])
                            In-Active
                        @else
                            Active
                        @endif
            ...
            <div class="text-end">
                ...
                @if($editFlag)
                    <button class="btn btn-primary my-3" id="submitButton" data-type="update" data-id="{{$ExamType['id']}}">Update</button>
                @else
        ...
        $(document).ready(function(){
            ...
            $("#submitButton").click(function() {
                ...
                if(editFlag && type == 'update'){
                    id = $(this).data("id");
                    reqUrl = '/admin/exam-types/'+id;
                    reqType = "PATCH";
                } else {
                    reqUrl = '/admin/exam-types';
                    reqType = "POST";
                }
                if(name){
                    $.ajax({
                        ...
                        success: function(data) {
                            if(data.success){
                                window.location.href = "/admin/exam-types";
                            }
                        },
                    ...
                    $("#formError").text("Please enter exam type name");
                }
            });
            // ready ends 
        });
