...
<body>
    ...
    <section class="p-4">
        <div class="my-4">
            <h1 class="text-dark my-2">Exams Types</h1>
            <p class="text-dark fs-5 my-3">Effectively manage exam types, covering first term and second term assessments, on the admin dashboard for streamlined oversight and organization.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Exam Types</h2>
                <a href="/admin/exam-types/create"><button class="btn rounded-2 text-uppercase create_new_btn text-white "><i class='bx bx-plus-circle mb-1 text-white'></i>Add new</button></a>
            </div>
            ...
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        ...
                        <th scope="col" class="font_bold w_180 fs-6 text-dark py-3">NAME</th>
                    ...
                    <tbody>
                        ...
                        @if($examTypes()['meta']['pagination']['count'] > 0)
                            @foreach($examTypes()['data'] as $examType)
                                <tr>
                                    <td class="py-4" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$examType['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4" data-label="exam">{{$examType['name']}}</td>
                                    ...
                                    <td class="py-4" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/exam-types/{{$examType['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item deleteExamType cursor-pointer" data-id="{{$examType['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($examTypes()['meta']['pagination']['per_page'] == 10)
                                    <option value="10" selected>10</option>
                                @else
                                    <option value="10">10</option>
                                @endif
                                @if($examTypes()['meta']['pagination']['per_page'] == 20)
                                    <option value="20" selected>20</option>
                                @else
                                    <option value="20">20</option>
                                @endif
                                @if($examTypes()['meta']['pagination']['per_page'] == 50)
                                    <option value="50" selected>50</option>
                                @else
                                    <option value="50">50</option>
                                @endif
                                @if($examTypes()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($examTypes()['meta']['pagination']['total'] > $examTypes()['meta']['pagination']['count'])
                                @isset($examTypes()['meta']['pagination']['links']['previous'])
                                    <span>
                                        <a href="{{$examTypes()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                                    </span>
                                @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$examTypes()['meta']['pagination']['current_page']}}</span> OF {{$examTypes()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($examTypes()['meta']['pagination']['total'] > $examTypes()['meta']['pagination']['count'])
                                @isset($examTypes()['meta']['pagination']['links']['next'])
                                    <span>
                                        <a href="{{$examTypes()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                                    </span>
                                @endisset
                            @endif
                        </div>
        ...
        $(document).ready(function() {
            ...
            $('#searchCode, #searchTitle').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/exam-types?" + testCode + testName;
            ...
            $("#select_status").on('change' ,function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testName = queryName ? "&name=" + queryName : "";
                    let url = "/admin/exam-types?" + testCode + testName + testStatus;
                    window.location.href = url;
                }
            })
            // row per page 
            $('#per_page_option').on('change', function (e) {
                ...
                var current_page = {!! str_replace("'", "\'", json_encode($examTypes()['meta']['pagination']['current_page'])) !!};
                if(perPage == 10){
