...
<head>
    ...
    @elseif(Auth::user()->hasRole('admin'))
        Admin Portal | {{ app(\App\Settings\SiteSettings::class)->app_name }}
    @endif
   </title>
    <!-- rel icon -->
...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Categories</h1>
            <p class="text-dark fs-5 my-2">Admins can effortlessly manage categories of learning for students. Explore more details in our documentation on <a href="admin/docs/master-data/manage-categories" class="text-primary text-decoration-none">manage categories</a>.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow my-4">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                ...
                <button data-bs-toggle="modal" data-bs-target="#newCategoryModal" class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr class="row_bg_light">
                            <td class="py-3" data-label="search">
                                <input type="text" id="searchCode"
                                    class="shadow-none form-control bg-light font_normal_bold placeholder_text select-custom"
                            ...
                            <td class="py-3" data-label="search"></td>
                        </tr>
                        @if(count($categories()['data']) > 0)
                            @foreach($categories()['data'] as $category)
                                <tr>
                                    <td class="py-3" data-label="code">
                                        <span
                                            class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            <span class="me-1">
                                                <svg class="copy me-1" fill="#ffffff" height="20px" width="20px"
                                                    style="width: 20px!important;" version="1.1" id="Layer_1"
                                                ...
                                                <svg class="tick me-1" fill="#ffffff" style="display:none;"
                                            ...
                                            <span class="questionCodeText">{{$category['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-3" data-label="exam">{{$category['name']}}</td>
                                    ...
                                    <td class="py-3" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                ...
                                                <li>
                                                    <a class="dropdown-item cursor-pointer delete" data-id="{{$category['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option"
                                ...
                                @if($categories()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($categories()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($categories()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($categories()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($categories()['meta']['pagination']['total'] > $categories()['meta']['pagination']['count'])
                            @isset($categories()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$categories()['meta']['pagination']['links']['previous']}}"><button
                                        class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$categories()['meta']['pagination']['current_page']}}</span>
                                OF {{$categories()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($categories()['meta']['pagination']['total'] > $categories()['meta']['pagination']['count'])
                            @isset($categories()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$categories()['meta']['pagination']['links']['next']}}"><button
        ...
        <div class="modal fade" id="newCategoryModal" aria-labelledby="modalToggleLabel" tabindex="-1" style="display: none;"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-dark" id="modalToggleLabel">Create New Category</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-input">
                            <div class="my-4">
                                ...
                                <input type="text" class="form-control mt-3" id="newCategoryName" placeholder="Enter Category Name">
                    ...
                    <div class="modal-footer">
                        <div class="w-100 text-end">
                            ...
                            <button class="btn btn-primary" id="createNewCategory">Create</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function(){
            ...
            $('#searchCode, #searchName').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/categories?" + testCode + testUser;
            ...
            $("#searchStatus").on('change', function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testUser = queryUser ? "&name=" + queryUser : "";
                    let url = "/admin/categories?" + testCode + testUser + testStatus;
                    window.location.href = url;
                }   
            })
            // row per page 
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($categories()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
            ...
            $("#createNewCategory").click(function(){
                ...
                if (CategoryName == "") {
                    $("#newCategoryName").addClass("border-danger");
                    $("#newCategoryName_error").removeClass("d-none");
                    $("#newCategoryName_error").html("The name field is required");
                } else {
                    $("#newCategoryName").removeClass("border-danger")
                    $("#newCategoryName_error").addClass("d-none")
                    $.ajax({
                        dataType: 'json',
                        type: "POST",
                        ...
                        data: {
                            ...
                            is_active: $('#flexSwitchCheckChecked').is(':checked'),
                            name: CategoryName,
