...
<head>
    ...
    <title>Exam Schedules - {{$exam['title']}}</title>
    <!-- rel icon -->
...
<body> 
        ...
        <div class="bg-white rounded-4 py-4 p-3 shadow my-4">
            <div class="row m-0">
                <div class="col-12 align-self-center">
                    <h1 class="h4 text-dark mb-0">Exam Schedules</h1>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody id="examSchedulesTable">
                        ...
                        @if(count($examSchedules()['data']) > 0)
                            @foreach($examSchedules()['data'] as $examSchedule)
                                <tr>
                                    <td class="py-4" data-label="serial"><span class="table_bg_primary text-white small p-2 bg-primary rounded-3">{{$examSchedule['code']}}</span></td>
                                    <td class="py-4" data-label="exam">{{$examSchedule['type']}}</td>
                                    <td class="py-4" data-label="exam">{{$examSchedule['starts_at']}}</td>
                                    <td class="py-4" data-label="exam">{{$examSchedule['ends_at']}}</td>
                                    <td class="py-4" data-label="status">
                                            <span class="p-2 px-3 rounded small 
                                            @if($examSchedule['status'] == 'Active')
                                                bg_label_published
                                            @elseif($examSchedule['status'] == 'Expired')
                                                bg_label_draft
                                            @elseif($examSchedule['status'] == 'Cancelled')
                                                bg_label_cancel
                                            @else
                                                bg-label-dark
                                            @endif
                                            ">{{$examSchedule['status']}}</span>
                                    </td>
                                    <td class="py-4" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/exams/{{$exam['id']}}/overall-report?schedule={{$examSchedule['id']}}">Analytics</a> 
                                                </li>
                                                <li>
                                                    <a class="dropdown-item delete_schedule cursor-pointer" data-exam-id="{{$exam['id']}}" data-schedule-id="{{$examSchedule['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($examSchedules()['meta']['pagination']['per_page'] == 10)
                                    <option value="10" selected>10</option>
                                @else
                                    <option value="10">10</option>
                                @endif
                                @if($examSchedules()['meta']['pagination']['per_page'] == 20)
                                    <option value="20" selected>20</option>
                                @else
                                    <option value="20">20</option>
                                @endif
                                @if($examSchedules()['meta']['pagination']['per_page'] == 50)
                                    <option value="50" selected>50</option>
                                @else
                                    <option value="50">50</option>
                                @endif
                                @if($examSchedules()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($examSchedules()['meta']['pagination']['total'] > $examSchedules()['meta']['pagination']['count'])
                                @isset($examSchedules()['meta']['pagination']['links']['previous'])
                                    <span>
                                        <a href="{{$examSchedules()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                                    </span>
                                @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$examSchedules()['meta']['pagination']['current_page']}}</span> OF {{$examSchedules()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($examSchedules()['meta']['pagination']['total'] > $examSchedules()['meta']['pagination']['count'])
                                @isset($examSchedules()['meta']['pagination']['links']['next'])
                                    <span>
                                        <a href="{{$examSchedules()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                                    </span>
                                @endisset
                            @endif
                        </div>
    ...
    <script>
        ...
        $('#per_page_option').on('change', function (e) {
            ...
            var current_page = {!! str_replace("'", "\'", json_encode($examSchedules()['meta']['pagination']['current_page'])) !!};
            if(perPage == 10){
        ...
        $(function(){
                            ...
                            if(data.success){
                                ...
                                let tdAction =  '<td><div class="dropdown border rounded">'+
                                                    ...
                                                    '<ul class="dropdown-menu dropdown-menu-end p-0" data-popper-placement="top-end">'+
                                                        '<li><a class="dropdown-item" href="/admin/exams/'+exam_id+'/overall-report?schedule='+data.schedule.id+'">Analytics</a></li>'+
                                                        '<li><a class="dropdown-item delete_schedule cursor-pointer" data-exam-id="'+exam_id+'" data-schedule-id="'+data.schedule.id+'">Delete</a></li>'+
                                                    '</ul>'+
                                                '</div></td>';
                                $("#examSchedulesTable").append('<tr>'+tdCode+tdType+tdStartsAt+tdEndsAt+tdStatus+tdAction+'</tr>')
                            }
                        },
            ...
            $(document).on('click', '.delete_schedule', function(){ 
                ...
                $.ajax({
                    type: "DELETE",
                    url: '/admin/exams/'+examId+'/schedules/'+scheduleId,
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(data) {
                        if(data.success){
                            ...
                            $("#newScheduleSuccess").empty();
                            $("#newScheduleSuccess").append('<div class="alert alert-success mt-3 shadow text-black">'+data.message+'</div>');
                            let recordLength = $("#examSchedulesTable").find("tr").length;
                            if(recordLength <= 1){
                                $('#examSchedulesTable').append('<tr id="noexamSchedule"><td class="text-center text-danger" colspan="6">No Records Found</td></tr>');
                            }
                        } else {
                            $("#newScheduleSuccess").empty();
                            $("#newScheduleSuccess").append('<div class="alert alert-danger mt-3 shadow">'+data.message+'</div>');
                        }
                    },
                    error: function(data, textStatus, errorThrown) {
                        console.log(textStatus);
