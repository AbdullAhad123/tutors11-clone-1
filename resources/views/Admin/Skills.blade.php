...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Topics</h1>
            <p class="text-dark fs-5 my-2">Admins and instructors can effortlessly manage, add, and update topics for any subject with just a click.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow my-4">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Manage Topics</h2>
                <button data-bs-toggle="modal" data-bs-target="#newSkillsModal" class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button>
            </div>

            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_180">CODE</th>
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_300">TOPIC</th>
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_300">SUBJECT</th>
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_160">Year</th>
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_180">TOTAL Q's</th>
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_160">STATUS</th>
                        <th scope="col" class="font_bold text-dark fs-5 py-3 w_160">ACTIONS</th>
                    </thead>
                    <tbody>
                        <tr class="row_bg_light">
                            ...
                            <td class="py-4 text-dark" data-label="search">
                                <select class="form-select" is="searchStatus">
                                ...
                                <tr>
                                    <td class="py-4 text-dark" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$skill['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4 text-dark" data-label="exam">{{$skill['name']}}</td>
                                    <td class="py-4 text-dark" data-label="completed">{{$skill['section']}}</td>
                                    <td class="py-4 text-dark" data-label="completed" class="text-nowrap">{{$skill['year']}}</td>
                                    <td class="py-4 text-dark" data-label="completed">{{$skill['ques_count']}} Q's</td>
                                    ...
                                    <td class="py-4 text-dark" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/skills/{{$skill['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item delete cursor-pointer" data-id="{{$skill['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($skills()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($skills()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($skills()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($skills()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($skills()['meta']['pagination']['total'] > $skills()['meta']['pagination']['count'])
                            @isset($skills()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$skills()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">Previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$skills()['meta']['pagination']['current_page']}}</span> OF {{$skills()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($skills()['meta']['pagination']['total'] > $skills()['meta']['pagination']['count'])
                            @isset($skills()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$skills()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
        ...
        <!-- new skill Modal  -->
        <div class="modal fade" id="newSkillsModal" aria-labelledby="modalToggleLabel" tabindex="-1" style="display: none;" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-input">
                            <div class="my-4">
                                ...
                                <input type="text" class="form-control mt-3" id="newSkillsName" placeholder="Enter Topic Name...">
                            ...
                            <div class="my-4">
                                ...
                                <textarea name="description" cols="40" rows="2" class="text_area_div my-3 form-control" placeholder="Enter Description..."
                    ...
                    <div class="modal-footer">
                        <div class="w-100 text-end">
                            ...
                            <button class="btn btn-primary" id="createNewSkill">Create Topic</button>
                        </div>
                    </div>
        ...
        $(document).ready(function(){
            ...
            $('#searchCode, #searchTitle').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/skills?" + testCode + testName;
            ...
            $("#searchStatus").on('change', function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testName = queryName ? "&name=" + queryName : "";
                    let url = "/admin/skills?" + testCode + testName + testStatus;
                    window.location.href = url;
                }
            })
            // row per page 
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($skills()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
