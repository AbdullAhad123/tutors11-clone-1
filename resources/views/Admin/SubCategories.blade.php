...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Years</h1>
            <p class="text-dark fs-5 my-2">Effortlessly manage, update, and track learning years in the admin portal for seamless administration.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow my-4">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Manage Categories</h2>
                <button data-bs-toggle="modal" data-bs-target="#newYearModal" class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_160">CODE</th>
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_160">NAME</th>
                                ...
                                <tr>
                                    <td class="py-4" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$subcategory['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4" data-label="exam">{{$subcategory['name']}}</td>
                                    <td class="py-4" data-label="exam">{{$subcategory['category']}}</td>
                                    <td class="py-4" data-label="exam">{{$subcategory['type']}}</td>
                                    <td class="py-4" data-label="exam">
                                    ...
                                    <td class="py-4" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/sub-categories/{{$subcategory['id']}}/map-section">Map Subject</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item" href="/admin/sub-categories/{{$subcategory['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item cursor-pointer delete" data-id="{{$subcategory['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($subCategories()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($subCategories()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($subCategories()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($subCategories()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($subCategories()['meta']['pagination']['total'] > $subCategories()['meta']['pagination']['count'])
                                @isset($subCategories()['meta']['pagination']['links']['previous'])
                                <span>
                                    <a href="{{$subCategories()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                                </span>
                                @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$subCategories()['meta']['pagination']['current_page']}}</span> OF {{$subCategories()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($subCategories()['meta']['pagination']['total'] > $subCategories()['meta']['pagination']['count'])
                                @isset($subCategories()['meta']['pagination']['links']['next'])
                                <span>
                                    <a href="{{$subCategories()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                                </span>
                                @endisset
                            @endif
                        </div>
        ...
        <div class="modal fade" id="newYearModal" aria-labelledby="modalToggleLabel" tabindex="-1" style="display: none;"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-dark" id="modalToggleLabel">Create New Year</h5>
                        <button type="button" id="closeNewUser" class="btn-close" data-bs-dismiss="modal"
                    ...
                    <div class="modal-body">
                        <div class="form-input">
                            <div class="my-4">
                                ...
                                <input type="text" class="form-control mt-3" id="newCategoryName" placeholder="Enter Year Name">
                                <span id="newCategoryName_error" class="text-danger small d-none"></span>
                            </div>
                            <div class="my-4">
                                <label for="category" class="text_black font_bold">Category</label>
                            ...
                            <div class="my-4">
                                <label for="type" class="text_black font_bold">Type</label>
                            ...
                            <div class="my-4">
                                <label>Short Description (Max. 160
                                    Characters)</label>
                                <textarea name="description" cols="40" rows="2" class="text_area_div my-3 form-control"
                                    style="max-height: 100px;" id="shortDescription"></textarea>
                            </div>
                    ...
                    <div class="modal-footer">
                        <div class="w-100 text-end">
                            ...
                            <button class="btn btn-primary" id="createNewSubCategory">Create</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function(){
            ...
            $('#searchCode, #searchName').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/sub-categories?" + testCode + testUser;
            ...
            $("#searchStatus").on('change', function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testUser = queryUser ? "&name=" + queryUser : "";
                    let url = "/admin/sub-categories?" + testCode + testUser + testStatus;
                    window.location.href = url;
                }   
            })
            // row per page 
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($subCategories()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
            ...
            $("#flexSwitchCheckChecked").change(function() {
                ...
                if(checked){
                    $(".active_inactive_text").text("Active");
                } else {
                    $(".active_inactive_text").text("In-active");
                }
            });
            // form filled 
            $("#flexSwitchCheckChecked, #newCategoryName, #category, #type, #shortDescription, #summernote").change(function() {
                $("#formError").text("");
            });
            // create row  
