...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">User Groups</h1>
            <p class="text-dark fs-5 my-2">Admin can manage user groups on website and portal. You can edit , update , delete and manage details of user groups in just one click. Read our <a href="/admin/docs/user-management/user-groups" target="_blank" class="text-primary text-decoration-none">docs</a> for quick start.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow my-4">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Manage Groups</h2>
                <button data-bs-toggle="modal" data-bs-target="#newuserGroupModal" class="btn rounded-2 text-uppercase create_new_btn text-white"><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold text-dark py-3 fs-6 w_200">CODE</th>
                        <th scope="col" class="font_bold text-dark py-3 fs-6 w_200">NAME</th>
                        <th scope="col" class="font_bold text-dark py-3 fs-6 w_180">VISIBILITY</th>
                        <th scope="col" class="font_bold text-dark py-3 fs-6 w_160">STATUS</th>
                        <th scope="col" class="font_bold text-dark py-3 fs-6 w_160">ACTIONS</th>
                    </thead>
                    <tbody>
                        <tr class="row_bg_light">
                            <td data-label="search">
                                <input type="text" id="searchCode" class="shadow-none form-control bg-light font_normal_bold placeholder_text select-custom" placeholder="Search Code">
                            </td>
                            <td data-label="search">
                            ...
                            <td data-label="search">
                                <select id="searchVisibility" class="form-select select_custom">
                                    <option selected>Search Visibility</option>
                                    <option value="1">Private</option>
                                    <option value="0">Public</option>
                                </select>
                            </td>
                            <td data-label="search">
                                <select id="searchStatus" class="form-select select_custom">
                                ...
                                <tr>
                                    <td data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$userGroup['code']}}</span>
                                        </span>
                                    </td>
                                    <td data-label="exam">{{$userGroup['name']}}</td>
                                    <td data-label="completed">{{$userGroup['visibility']}}</td>
                                    ...
                                    <td data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/user-groups/{{$userGroup['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item cursor-pointer delete" data-id="{{$userGroup['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($userGroups()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($userGroups()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($userGroups()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($userGroups()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($userGroups()['meta']['pagination']['total'] > $userGroups()['meta']['pagination']['count'])
                            @isset($userGroups()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$userGroups()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$userGroups()['meta']['pagination']['current_page']}}</span> OF {{$userGroups()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($userGroups()['meta']['pagination']['total'] > $userGroups()['meta']['pagination']['count'])
                            @isset($userGroups()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$userGroups()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
        ...
        <div class="modal fade" id="newuserGroupModal" aria-labelledby="modalToggleLabel" tabindex="-1" style="display: none;" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <!-- header  -->
                    <div class="modal-header">
                        <h5 class="modal-title text-dark" id="modalToggleLabel">New User Group</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-input">
                            <div class="my-4">
                                ...
                                <input type="text" class="form-control mt-3 " placeholder="Name" id="newUserGroupName">
                            ...
                            <div class="my-4">
                                <div class="row m-0 align-items-center justify-content-between">
                                    <div class="col-10">
                                        ...
                                        <p class="my-2">Active (Shown Everywhere). In-active (Hidden
                                            Everywhere).</p>
                                    </div>
                                    <div class="col-2 ">
                                        <div class="form-check form-switch text-center">
                                            <input class="form-check-input shadow-none border border-dark btn-lg" type="checkbox"
                                                role="switch" id="active_inactive_switch" checked>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="my-4">
                                <div class="row m-0 align-items-center justify-content-between">
                                    <div class="col-10">
                                        <label for="name" class="text_black font_bold ">
                                            <!-- printing -->
                                            <span class="public_private_text">Public</span> Group
                                        </label>
                                    ...
                                    <div class="col-2 ">
                                        <div class="form-check form-switch text-center">
                                            <input class="form-check-input shadow-none border border-dark btn-lg" type="checkbox"
                                                role="switch" id="public_private_check">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="my-4">
                                <div id="NewUserFormError" class="font-size-13 text-danger"></div>
                                <div class="text-end">
                                    <button class="btn btn-primary p-2 px-3" id="createUserGroup">Create User Group</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        $(document).ready(function(){
            ...
            $('#searchCode, #searchName').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/user-groups?" + testCode + testUser;
                    window.location.href = url;
                }
            });
            // on change searching 
            $("#searchVisibility").on('change', function() {
                let selectVisibility = $(this).val();
                if (selectVisibility != "Search Visibility") {
                    let testVisibility = "&visibility=" + selectVisibility;
                    let queryCode = $("#searchCode").val();
                    let queryUser = $("#searchName").val();
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testUser = queryUser ? "&name=" + queryUser : "";
                    let url = "/admin/user-groups?" + testCode + testUser + testVisibility;
                    window.location.href = url;
                }
            });
            // on change searching 
            $("#searchStatus").on('change', function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testCode = queryCode ? "&code=" + queryCode : "";
                    let testUser = queryUser ? "&name=" + queryUser : "";
                    let url = "/admin/user-groups?" + testCode + testUser + testStatus;
                    window.location.href = url;
                }
            });
            // row per page 
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($userGroups()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
            ...
            $("#createUserGroup").click(function(){
                ...
                if (name == "") {
                    $("#newUserGroupName").addClass("border-danger");
                    $("#newUserGroupName_error").removeClass("d-none");
                    $("#newUserGroupName_error").html("The name field is required");
                } else {
                    $("#newUserGroupName").removeClass("border-danger")
                    $("#newUserGroupName_error").addClass("d-none")
                    $.ajax({
                        dataType: 'json',
                        type: "POST",
            ...
            // text toggle 
            $("#public_private_check").click(function(){
                if ($(this).is(':checked')) {
                    $(".public_private_text").html("Private")
                }
                else {
                    $(".public_private_text").html("Public")
                }
            });
            // delete  
