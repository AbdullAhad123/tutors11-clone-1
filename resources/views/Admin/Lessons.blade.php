...
<body>
    ...
    <section class="p-4">
        ...
        <div class="my-4">
            <h1 class="text-dark my-2">Lesson</h1>
            <p class="text-dark my-2 fs-5">Seamlessly manage lessons with our Lesson Management Hub. Admins and instructors can easily add, update, and oversee lesson details for effective course management.</p>
        </div>
        ...
        <div class="bg-white rounded-4 shadow">
            <div class="align-items-center d-flex justify-content-between mb-4 p-3 px-4">
                <h2 class="h4 text-dark m-0">Lessons</h2>
                <a href="/admin/lessons/create"><button class="btn rounded-2 text-uppercase create_new_btn text-white "><i class='bx bx-plus-circle mb-1 text-white'></i>Create new</button></a>
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_160">CODE</th>
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_300">TITLE</th>
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_180">SUBJECT</th>
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_200">TOPIC</th>
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_200">SUB-TOPIC</th>
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_160">STATUS</th>
                        <th scope="col" class="font_bold fs-6 text-dark py-3 w_180">ACTIONS</th>
                    </thead>
                    <tbody>
                                ...
                                <tr>
                                    <td class="py-4" data-label="code">
                                        <span class="align-items-center bg-primary cursor-pointer d-inline-flex px-2 questionCode rounded-3 small table_bg_primary text-white">
                                            ...
                                            <span class="questionCodeText">{{$lesson['code']}}</span>
                                        </span>
                                    </td>
                                    <td class="py-4" data-label="question">{{$lesson['title']}}</td>
                                    <td class="py-4" data-label="section">{{$lesson['section']}}</td>
                                    <td class="py-4" data-label="skill">{{$lesson['skill']}}</td>
                                    <td class="py-4" data-label="topic">{{$lesson['topic']}}</td>
                                    ...
                                    <td class="py-4" data-label="actions">
                                        <div class="border d-inline-flex dropdown rounded shadow-sm">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end p-0">
                                                <li>
                                                    <a class="dropdown-item" href="/admin/lessons/{{$lesson['id']}}/edit">Edit</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item delete cursor-pointer" data-id="{{$lesson['id']}}">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg_primary_label px-1">
                                @if($lessons()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($lessons()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($lessons()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($lessons()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($lessons()['meta']['pagination']['total'] > $lessons()['meta']['pagination']['count'])
                            @isset($lessons()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$lessons()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$lessons()['meta']['pagination']['current_page']}}</span> OF {{$lessons()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($lessons()['meta']['pagination']['total'] > $lessons()['meta']['pagination']['count'])
                            @isset($lessons()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$lessons()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
        ...
        $(document).ready(function () {
            ...
            $('#searchCode, #searchTitle , #searchSection , #searchSkill , #searchTopic').on('keyup', function(e) {
                ...
                if (code == 13) {
                    ...
                    let url = "/admin/lessons?" + testCode + testName + testSection + testSkill + testTopic;
            ...
            $("#searchStatus").on('change', function() {
                ...
                if (selectStatus != "Search Status") {
                    ...
                    let testSkill = querySkill ? "&skill=" + querySkill : "";
                    let testTopic = queryTopic ? "&topic=" + queryTopic : "";
                    let url = "/admin/lessons?" + testCode + testName + testSection + testSkill + testTopic + testStatus;
                    window.location.href = url;
                }
            })
            // row per page 
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($lessons()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
