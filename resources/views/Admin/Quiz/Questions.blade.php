...
<body> 
    ...
    <section class="p-4">
        ...
        <div class="bg-white rounded-4 py-4 p-3 shadow">
            <div class="row m-0">
                <div class="col-lg-5 col-md-12 align-self-center my-2">
                    <h1 class="h4 text-dark mb-0">Quiz Questions</h1>
        ...
        <div class="bg-white rounded-4 py-4 my-4 p-2 text-dark shadow">
                    ...
                    <div class="col-md-8 col-12 my-3">
                        <div class="alert_success mb-4 p-3 rounded-3 shadow">
                            ...
                            <div class="nav mt-2" role="tablist">
                                <button class="bg-transparent border-0 text-primary px-1 active" id="view-questions-tab" data-bs-toggle="tab" data-bs-target="#view-questions" type="button" role="tab" aria-controls="view-questions" aria-selected="true">View Questions</button>
                                <span class="align-self-center font-black"> | </span>
                                <button class="bg-transparent border-0 text-primary px-1" id="add-questions-tab" data-bs-toggle="tab" data-bs-target="#add-questions" type="button" role="tab" aria-controls="add-questions" aria-selected="false">Add Questions</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-body-tertiary border p-3 rounded-4 shadow-sm tab-content">
                            <div class="tab-pane fade active show" id="view-questions" role="tabpanel" aria-labelledby="view-questions-tab">
                                <div>
                                    <div class="text-body text-center text-dark fs-5"><span id="countQuestions"></span> items match the selected criteria.</div>
                                    ...
                                    <div id="quizfetchedQuestions"></div>
                                </div>
                            </div>
                            <div class="tab-pane fade" id="add-questions" role="tabpanel" aria-labelledby="add-questions-tab">
                                <div>
                                    <div class="text-body text-center text-dark fs-5"><span id="countAvailableQuestions"></span> items match the selected criteria.</div>
                                    <div id="noAvailableQuestions" class="text-center my-4">
                                        <img src="{{url('images/no_record_found.png')}}" width="200" height="auto" alt="no records found">
                                        <h2 class="h5 text-center text-dark my-1">No Questions</h2>
                                    </div>
                                    <div id="quizAvailableQuestions"></div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="container px-1 random_div">
                <form action="{{ route('quizzes.randomly.add_questions', ['quiz' => $quiz['id']]) }}" method="post">
                @csrf
                    <div class="mb-3">
                            <label class="form-label my-2" for="limit"><span class="text-dark">Choose Number Of Questions</span> (<span id="total">0</span> Questions)</label>
                            <input type="range" id="range" name="limit" class="form-control-range w-75 shadow-none w-100 queNumRanger" min="0" max="50" value="0">
                        </div>
                    <div class="text-end">
    ...
    <script>
        ...
        function addQuestions() {
            $.ajax({
                type: "GET",
                url: '/admin/quizzes/'+quiz_id+'/fetch-available-questions',
                data: {
                ...
                success: function(data) {
                    ...
                    if(countQuestions > 0) {
                        ...
                        addQuestionsToView($("#quizAvailableQuestions"), questions, data['questions'].meta.pagination);
                    }
                },
        ...
        function fetchQuestions() {
            $.ajax({
                type: "GET",
                url: '/admin/quizzes/'+quiz_id+'/fetch-questions',
                data: {
                    question_types: questionTypes,
                    difficulty_levels: difficultyLevel,
                    code: codeSearcher,
                ...
                success: function(data) {
                    ...
                    if(countQuestions > 0) {
                        $("#noQuestions").hide();
                        addQuestionsToView($("#quizfetchedQuestions"), questions, data['questions'].meta.pagination);
                    }
                },
                error: function(data, textStatus, errorThrown) {
                    console.log(textStatus);
        ...
        function addQuestionsToView(_this, questions, pagination) {
            ...
            questions.forEach(function (question, i) {
                ...
                let tab =   '<div class="px-2 py-3 my-3 bg-white question font-size-13" data-code="'+question.code+'" data-difficulty="'+question.difficulty+'" data-type="'+question.question_type+'">' +
                                ...
                                '<div class="my-4">'+question.question+'</div>'+
                ...
                if(id_selector == 'quizfetchedQuestions'){
        ...
        $(document).on("click",".addavailableQuestion", function(e){
            ...
            $.ajax({
                type: "POST",
                url: '/admin/quizzes/'+quiz_id+'/add-question',
                data: {
        ...
        $(document).on("click",".removeQuestion", function(e){
            ...
            $.ajax({
                type: "POST",
                url: '/admin/quizzes/'+quiz_id+'/remove-question',
                data: {
                    question_id: questionId,
                    _token: '{{ csrf_token() }}'
                },
