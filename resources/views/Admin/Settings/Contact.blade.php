        ...
        <div class="table-responsive">
                <table class="table">
                    <thead class="bg-body-secondary">
                        ...
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_180">NAME</th>
                        ...
                        <th scope="col" class="font_bold text-dark fs-6 py-3 w_200">MESSAGE</th>
                    ...
                    <tbody>
                        <tr class="row_bg_light">
                            ...
                            <td class="py-4">
                                <input type="text" data-url="phone" class="search shadow-none form-control bg-light font_normal_bold placeholder_text" placeholder="Search Phone">
                            </td>
                            <td class="py-4">
                        ...
                        @if(count($contacts()['data']) > 0)
                            @foreach($contacts()['data'] as $key => $contact)
                                <tr>
                                    ...
                                    <td class="py-4 text-dark">{{$contact['name']}}</td>
                                    <td class="py-4 text-dark">{{$contact['phone']}}</td>
                                    <td class="py-4 text-dark">{{$contact['email']}}</td>
                                    <td class="py-4 text-dark">{{$contact['message']}}</td>
                                    <td class="py-4 text-dark">{{$contact['date']}}</td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
                    ...
                    <div class="px-3 py-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <select name="row_selection" id="per_page_option" class="ms-2 btn btn-primary bg-label-primary px-1">
                                @if($contacts()['meta']['pagination']['per_page'] == 10)
                                <option value="10" selected>10</option>
                                @else
                                <option value="10">10</option>
                                @endif
                                @if($contacts()['meta']['pagination']['per_page'] == 20)
                                <option value="20" selected>20</option>
                                @else
                                <option value="20">20</option>
                                @endif
                                @if($contacts()['meta']['pagination']['per_page'] == 50)
                                <option value="50" selected>50</option>
                                @else
                                <option value="50">50</option>
                                @endif
                                @if($contacts()['meta']['pagination']['per_page'] == 100)
                        ...
                        <div class="align-self-center">
                            @if($contacts()['meta']['pagination']['total'] > $contacts()['meta']['pagination']['count'])
                            @isset($contacts()['meta']['pagination']['links']['previous'])
                            <span>
                                <a href="{{$contacts()['meta']['pagination']['links']['previous']}}"><button class="btn btn-sm bg-label-dark">previous</button></a>
                            </span>
                            @endisset
                            @endif
                            <span class="mx-2">
                                PAGE <span id="page_active">{{$contacts()['meta']['pagination']['current_page']}}</span> OF {{$contacts()['meta']['pagination']['total_pages']}}
                            </span>
                            @if($contacts()['meta']['pagination']['total'] > $contacts()['meta']['pagination']['count'])
                            @isset($contacts()['meta']['pagination']['links']['next'])
                            <span>
                                <a href="{{$contacts()['meta']['pagination']['links']['next']}}"><button class="btn btn-sm bg-label-dark">Next</button></a>
                            </span>
                            @endisset
                            @endif
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#per_page_option').on('change', function(e) {
                ...
                var current_page = {!!str_replace("'", "\'", json_encode($contacts()['meta']['pagination']['current_page'])) !!};
                if (perPage == 10) {
