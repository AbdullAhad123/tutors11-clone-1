...
<div class="bg-white dark:bg-gray-900 overflow-auto">
    <div x-data="{ open: false }" @keydown.window.escape="open = false" class="bg-white shadow-sm">
        <!-- Mobile menu -->
        <div x-show="open" class="fixed inset-0 flex z-40 lg:hidden" x-description="Off-canvas menu for mobile, show/hide based on off-canvas menu state." x-ref="dialog" aria-modal="true">
            ...
            <div x-show="open" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="ltr:-translate-x-full rtl:translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="ltr:-translate-x-full rtl:translate-x-full" x-description="Off-canvas menu, show/hide based on off-canvas menu state."
                ...
                <div class="border-t border-gray-200 py-6 px-4 space-y-6">
                    <div class="flow-root">
                        <a href="{{ route('user/login') }}" class="-m-2 p-2 block font-medium text-gray-900">{{ __('Login') }}</a>
                    </div>
                </div>
            @endguest
            @auth 
                <div class="border-t border-gray-200 py-6 px-4 space-y-6">
                    <div class="flow-root">
                        <a href="{{ route('home') }}" class="-m-2 p-2 block font-medium text-gray-900">{{ __('Go to Dashboard') }}</a>
                    </div>
                </div>
            @endauth
            </div>
                    ...
                    <div class="h-16 sm:h-20 flex items-center">
                        ...
                        <div class="ltr:mr-4 rtl:ml-4 flex ltr:lg:ml-0 rtl:lg:ml-0">
                            <a href="{{ route('welcome') }}">
                                ...
                                <img class="h-8 w-auto" style="border: none;" src="{{ asset('storage/'.$siteSettings->logo_path) }}" alt="{{ $siteSettings->app_name }}">
                            </a>
                        </div>

                        <!-- Menus -->
                        <div class="hidden lg:ltr:ml-8 lg:rtl:mr-8 lg:block lg:self-stretch">
                            <div class="flex h-full mx-52 rtl:space-x-reverse space-x-8">
                                <a href="{{ url('/') }}#explore" class="flex items-center font-medium text-gray-700 hover:text-gray-800 navbar_links">{{ __('Explore') }}</a>
                                <a href="{{ route('learn_practice') }}" class="flex items-center font-medium text-gray-700 hover:text-gray-800 navbar_links">{{ __('Learn & Practice') }}</a>
                                <a href="{{ route('quiz_dashboard') }}" class="flex items-center font-medium text-gray-700 hover:text-gray-800 navbar_links">{{ __('Quizzes') }}</a>
                                <a href="{{ route('pricing') }}" class="flex items-center font-medium text-gray-700 hover:text-gray-800 navbar_links">{{ __('Pricing') }}</a>
                            </div>
                        </div>
                        ...
                        <div class="ltr:ml-auto rtl:mr-auto flex items-center">
                            ...
                            @guest
                                <div class="hidden ltr:lg:ml-8 rtl:lg:mr-8 lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                                    <a href="{{ route('user/login') }}" class="flex items-center justify-center px-8 py-2 border border-transparent rounded-sm shadow-sm text-sm font-medium text-white bg-dark hover:opacity-90">
                                        {{ __('Login') }}
                                    </a>
                                </div>
                            @endguest
                            @auth
                                <div class="hidden ltr:lg:ml-8 rtl:lg:mr-8 lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                                    <a href="{{ route('home') }}" class="flex items-center justify-center px-8 py-2 border border-transparent rounded-sm shadow-sm text-sm font-medium text-white bg-dark hover:opacity-90">
                                        {{ __('Go to Dashboard') }}
                                    </a>
                                </div>
                            @endauth
