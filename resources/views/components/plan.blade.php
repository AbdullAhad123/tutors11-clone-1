<div class="w-full sm:w-80 relative bg-white border rounded-lg shadow-sm divide-y divide-gray-200 {{ $plan['popular'] ? 'border-secondary' : 'border-gray-200' }}">
    <div class="p-6">
        ...
        <div class="flex gap-2 items-center justify-start">
            <h2 class="text-lg leading-6 font-semibold text-dark">{{ $plan['name'] }}</h2>
        ...
        <a href="{{ route('checkout', ['plan' => $plan['code']]) }}" class="mt-4 block w-full bg-gray-800 border rounded-md py-2 font-semibold text-white text-center hover:opacity-80" style="background-color: #8A9700;">
            {{ __('Buy Plan') }}
        </a>
    </div>
