@if($homePageSettings->enable_top_bar)
    <section class="bg-dark" x-data="{ barOpen: true }">
            ...
            <div class="flex items-center justify-between flex-wrap">
                ...
                <span class="flex p-2 rounded-md">
                    ...
                    <span class="md:hidden">
                        ...
                        <span class="hidden md:inline" style="position: relative; top:4px;">
                        {{ $topBar->message }}
                    </span> 
                    </p>
                </div>
                <div class="order-3 mt-2 flex-shrink-0 w-full sm:order-2 sm:mt-0 sm:w-auto">
                    <a href="{{ $topBar->button_link }}" target="_blank"
                       class="flex items-center justify-center px-3 py-1 border border-transparent rounded-sm shadow-sm text-sm font-medium text-white hover:opacity-70" style="position: relative; left:10px; background-color:#8A9700; bottom:2px; border-bottom-right-radius: 20px;">
                        {{ $topBar->button_text }}
                    </a>
                </div>
