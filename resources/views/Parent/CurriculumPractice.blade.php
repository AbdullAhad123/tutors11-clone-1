...
<head>
    ...
    <title>Practises - Curiculum</title>
    <!-- rel icon -->
    ...
    <style>
        ...
        .total_card {
            background: radial-gradient(#4e9fff, #1f6ecb) !important;
        }

        .attempted_card {
            background: radial-gradient(#6ccd76, #459662) !important;
        }
        ...
        .curriculum_progress_card {
            box-shadow: 0px 4px 8px 0px #00000052;
        }

        .curriculum_session_card {
            height: auto;
            width: auto;
            background: linear-gradient(160deg, #000000a3, #00000012), url('../images/currculum_sets_cards_bg.jpg') center;
            background-size: cover;
        }

        .selected_curriculum_session_card {
        ...
        .step_svg{min-width:40px;width: 40px;}
    </style>
</head>
...
<body>
    ...
    <nav class='mb-4'
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item fw-light active small text-primary" aria-current="page">Curriculum - Practises
            </li>
        </ol>
    </nav>

    <!-- curriculum  -->
    <section class="session_curriculum_section">
        <h1 class="display-5 text-capitalize my-3">Exams Curriculum</h1>
        <div class="row m-0 my-3 justify-content-center">
            <div class="col-lg-4 col-md-6 col-sm-10 col-12 mx-auto my-2">
                <a href="{{url('curriculum/exams')}}" class="text-decoration-none">
                    <div
                        class="h-100 align-items-center curriculum_session_card d-flex p-lg-4 p-md-4 p-3 rounded-3 shadow">
                        <!-- <img src="{{url('images/total_exams_img.png')}}" style='filter: invert();' height='50' width='50' alt="exams icon" class="me-2 step_svg"> -->
                        ...
                        <div class="my-2">
                            <h2 class="text-white h4 mb-2">Exams Curriculum</h2>
                            <p class="text-white my-2">Track Your Exam Sessions Reports.</p>
                        </div>
                    </div>
                </a>
            </div>
            ...
            <div class="col-lg-4 col-md-6 col-sm-10 col-12 mx-auto my-2">
                <a href="{{url('curriculum/quizzes')}}" class="text-decoration-none">
                    <div class="h-100 align-items-center curriculum_session_card d-flex p-lg-4 p-md-4 p-3 rounded-3 shadow">
                ...
                <div class="nav nav-tabs justify-content-center" id="nav-tab" role="tablist">
                    ...
                    <a href="{{url('curriculum/quizzes')}}">
                        <button class="nav-link w-auto tab_navigation_buttons" id="nav-quiz-tab" data-bs-toggle="tab" data-bs-target="#nav-quiz" type="button" role="tab" aria-controls="nav-quiz" aria-selected="false">
                            <img src="{{url('images\quiz.png')}}" width="50px" alt="Quiz Image">
                            <h6 class="text-center text-dark m-0 mt-2">Quiz</h6>
                        </button>
                    </a>
                    <a href="{{url('curriculum/practices')}}">
                        <button class="nav-link w-auto tab_navigation_buttons" id="nav-practice-tab" data-bs-toggle="tab" data-bs-target="#nav-practice" type="button" role="tab" aria-controls="nav-practice" aria-selected="false">
                            <img src="{{url('images\practice.png')}}" width="50px" alt="Practice Image">
        ...
        @endif -->
        <div class="p-2 py-5 my-3">
            <div class="row m-0">
                @foreach ($data as $curriculum)
                    <div class="col-12 col-lg-4 col-md-4 col-sm-6 my-3">
                        <div class="bg-white rounded-5 curriculum_progress_card p-3">
                            <h2 class="h3 mb-4 my-3 text-center text-dark text-uppercase text-truncate" style="letter-spacing: 1.1px;">
                                {{$curriculum['title']}}</h2>
                            <canvas class="m-auto" id="myChart-{{ $curriculum['key'] }}"></canvas>
                        </div>
                    </div>
                @endforeach
    ...
    <!-- additional  -->
    <section class="additional_detailss_section">
        <h2 class="h1 text-capitalize my-3">Additional Details</h2>
        <div class="row m-0 align-items-center my-4">
            <div class="col-lg-6 col-md-6 col-sm-12 col-12 my-2">
                <div class="p-4 rounded-4 session_card overflow-hidden shadow position-relative total_card">
                    <div class="d-flex align-items-center">
                        <img src="{{url('images\total_exams_img.png')}}" height="50" width="50"
                            alt="Total session icon">
                        <h2 class="h3 m-0 ms-2 text-white">Total Practise Sessions</h2>
                    </div>
                    <h2 class="h4 mt-3 text-white">{{$total_session}} Sessions</h2>
                    <img src="{{url('images\cube.png')}}" height='80' width='80'
                        alt="exam cubes background" class="session_bg_image">
                </div>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-12 col-12 my-2">
                <div class="p-4 rounded-4 session_card overflow-hidden shadow position-relative attempted_card">
                    <div class="d-flex align-items-center">
                        <img src="{{url('images\attempt_exams_img.png')}}" height="50" width="50"
                            alt="Atempted session icon">
                        <h2 class="h3 m-0 ms-2 text-white">Atempted Practise Sessions</h2>
                    </div>
                    <h2 class="h4 mt-3 text-white">{{$done_session}} Sessions</h2>
                    <img src="{{url('images\cube.png')}}" height='80' width='80'
                        alt="exam cubes background" class="session_bg_image">
                </div>
            </div>
    ...
    @foreach ($data as $curriculum)
        <script>
            var curriculumtitle{{ $curriculum['key'] }} = "{!! addslashes($curriculum['title']) !!}";
            var curriculumpercentage{{ $curriculum['key'] }} = {{ $curriculum['percentage'] }};
            var curriculumcolor{{ $curriculum['key'] }} = "{!! addslashes($curriculum['color'][0]) !!}";
            var curriculumcolor1{{ $curriculum['key'] }} = "{!! addslashes($curriculum['color'][1]) !!}";
            const ctx{{ $curriculum['key'] }} = document.getElementById('myChart-{{ $curriculum['key'] }}');
            new Chart(ctx{{ $curriculum['key'] }}, {
                type: 'doughnut',
                data: {
                    labels: ['Complete', 'Incomplete'],
                    datasets: [{
                        label: curriculumtitle{{ $curriculum['key'] }},
                        data: [curriculumpercentage{{ $curriculum['key'] }}, 100 - curriculumpercentage{{ $curriculum['key'] }}],
                        backgroundColor: [ curriculumcolor{{ $curriculum['key'] }}, curriculumcolor1{{ $curriculum['key'] }}],
                    }]
                },
                options: {
                    aspectRatio: 1,
                ...
                plugins: [{
                    ...
                    beforeDraw: function (chart, a, b) {
                        ...
                        var text = parseFloat("{{ $curriculum['percentage'] }}").toFixed(0) + '%';
