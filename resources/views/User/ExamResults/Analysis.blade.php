...
<body>
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap my-4">
        <h1 class="text-dark my-2 h2">Exam Results</h1>
        <a href="/exams" class="align-items-center d-inline-flex my-2 text-decoration-underline text-primary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="#006ae7" class="me-1"><path d="M21 11H6.414l5.293-5.293-1.414-1.414L2.586 12l7.707 7.707 1.414-1.414L6.414 13H21z"></path></svg> Back to Exams</a>
    </div>

    <nav>
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            <a href="{{ $steps[0]['url'] }}" class="nav-link w-auto active" id="nav-{{ $steps[0]['key'] }}-tab">{{
                $steps[0]['title'] }}</a>
            <a href="{{ $steps[1]['url'] }}" class="nav-link w-auto" id="nav-{{ $steps[1]['key'] }}-tab">{{
                $steps[1]['title'] }}</a>
            <a href="{{ $steps[2]['url'] }}" class="nav-link w-auto" id="nav-{{ $steps[2]['key'] }}-tab">{{
                $steps[2]['title'] }}</a>
        </div>
    </nav>

    <div class="tab-content bg-white mb-5" id="nav-tabContent">
                ...
                <div class="col-12 col-sm-6 col-lg-3 px-3 position-relative mt-2">
                    <div class="row bg-white rounded-3 py-3 px-2 h-100 shadow-sm">
                        ...
                        <div class="col-6 text-end col-6 fs-4 align-self-center">
                            @if ($session['results']['pass_or_fail'] == 'Failed')
                            <p class="mb-2 text-danger">
                                {{ $session['results']['score'] < 0 ? 0 : $session['results']['score'] }}</p>
                                    @else
                                    <p class="mb-2 text-green">
                                        {{ $session['results']['score'] < 0 ? 0 : $session['results']['score'] }}</p>
                                            @endif
            ...
            <div class="justify-content-center row">
                <div class="col-12 col-sm-6 col-md-4 mt-3">
                    <div class="row bs-gutter-x-0 bg-white rounded-3 py-4 px-3 shadow-sm">
                        <p class="text-center text-dark">TOTAL {{ $session['results']['total_questions'] }} QUESTIONS
                        </p>
                ...
                <div class="col-12 col-sm-6 col-md-4 mt-3">
                    <div class="row bs-gutter-x-0 bg-white rounded-3 py-4 px-3 shadow-sm">
                        <p class="text-center text-dark">TOTAL
                            {{ number_format($session['results']['total_duration'], 1) }} Minutes</p>
                            ...
                            <table class="table table-bordered mb-0">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="py-2 px-3 text-end">-{{ $session['results']['marks_deducted'] }}
                                        </td>
                                    </tr>
                                    <tr>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered mb-0">
                        ...
                        <tbody>
                            ...
                            @if(isset($section['results']))
                                <tr class="table_row">
                                    <td>
                                        <?php echo $i; ?>
                                    </td>
                                    <td>{{ $section['name'] }}</td>
                                    <td>{{ $section['results']['score'] }}/{{ $section['results']['total_marks'] }}
                                    </td>
    ...
    <script>
        ...
        var total_question_data = [{{ $session['results']['correct_answered_questions'] }},
        {{ $session['results']['wrong_answered_questions'] }}, {{ $session['results']['unanswered_questions'] }}
        ];
        var total_time_data = [{{ $session['results']['time_taken_for_correct_answered']['seconds'] }},
        {{ $session['results']['time_taken_for_wrong_answered']['seconds'] }},
        {{ $session['results']['time_taken_for_other']['seconds'] }}
        ];
        var total_question_chart_data = {
            ...
            datasets: [{
                ...
                hoverBackgroundColor: [chartColors['transparent'][0], chartColors['transparent'][1],
                chartColors['transparent'][2]
                ]
            }]
        };
        var totalQuestionChart = new Chart(document.getElementById('myQuestionChart'), {
            ...
            plugins: [{
                ...
                beforeDraw: function (chart, a, b) {
                    ...
                    var text =
                        "{{ $session['results']['answered_questions'] }}/{{ $session['results']['total_questions'] }} Answered"
        ...
        var totalTimeChart = new Chart(document.getElementById('myTimeChart'), {
            ...
            plugins: [{
                ...
                beforeDraw: function (chart, a, b) {
                    ...
                    // session.results.answered_questions+'/'+session.results.total_questions+' '+__('Answered');,
                    var text =
                        "{{ $session['results']['total_time_taken']['detailed_readable'] }} Spent"
                    textX = Math.round((width - ctx.measureText(text).width) / 2),
                        textY = height / 2;
