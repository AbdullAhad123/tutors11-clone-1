...
<body>
    ...
    <!-- this is the section id.. if you remove it the browser will show up some errors -->
    <!-- it was place in navtabs where (navtabs) have written -->

    <!-- <div class="">
        @foreach ($sections as $key => $section)
            <button class="btn btn-primary w-px-200 section-btn" data-id="{{$section['id']}}"
                data-section_id="{{$section['section_id']}}"> {{$section['name']}} 
                @if(isset($section['results']))
                    <span class="text-dark bg-white px-1 rounded-1 fs-tiny d-inline-block translate-middle-y">+{{$section['results']['score']}}</span>
                @endif
            </button>
        @endforeach
    </div> -->
    ...
    <div clas="mb-5">
        <div class="bg-white p-lg-3 p-md-3 p-2">
                    ...
                    <div class="practice_card rounded-3 position-relative">
                        <div class="practice_title practice_grid_child justify-content-center">
                            <img src="{{url('images\question_answered.png')}}" class="practice_results_image" height="40"
                                width="40" alt="question attempted" style="-webkit-user-drag: none; -webkit-user-select: none;">
                        </div>
                        <div class="practice_progress practice_grid_child my-1">
                            <div class="practice_progress_parent ms-2" style="width: 90%;">
                                <span class="fs-6 text-dark"><span class="fs-4 fw-bold">{{$correct_answered_questions}}</span>/{{$exam['total_questions']}}</span>
                    ...
                    <div class="practice_card rounded-3 position-relative">
                        <div class="practice_title practice_grid_child justify-content-center">
                            <img src="{{url('images\darts.png')}}" class="practice_results_image" height="40" width="40"
                                alt="accuracy image" style="-webkit-user-drag: none; -webkit-user-select: none;">
                        </div>
                        <div class="practice_progress practice_grid_child my-1">
                            <div class="practice_progress_parent ms-2" style="width: 90%;">
                                <span class="fs-6 text-dark"><span class="fs-4 fw-bold">{{$total_section_accuracy}}</span>&nbsp;%</span>
                    ...
                    <div class="practice_card rounded-3 position-relative">
                        <div class="practice_title practice_grid_child justify-content-center">
                            <img src="{{url('images\timer.png')}}" class="practice_results_image" height="40" width="40"
                                alt="time spent image" style="-webkit-user-drag: none; -webkit-user-select: none;">
                        </div>
                        <div class="practice_progress practice_grid_child my-1">
                            <div class="practice_progress_parent ms-2" style="width: 90%;">
                                <span class="fs-6 text-dark"><span class="fs-4 fw-bold">{{$total_time_taken}}</span>&nbsp;sec</span>
            ...
            <div class="question_solutions my-5">
                ...
                @foreach($sections_questions as $sectionkey => $section)
                    <h2 class="text-center text-dark">{{$section['section']['name']}}</h2>
                    @foreach($section['questions']['data'] as $key => $question)
                        <div class="question_button_container my-3">
                            ...
                            <div class="w-100">
                                <button class="btn questions_button 
                                    ...
                                    @endif
                                    p-2 w-100 rounded-1" type="button" data-bs-toggle="collapse" data-bs-target="#question_solution_{{++$sectionkey}}{{++$key}}" aria-expanded="false" aria-controls="question_solution_{{$sectionkey}}{{$key}}">
                            ...
                            <!-- answer body  -->
                            <div class="collapse border border-1 mt-2" id="question_solution_{{$sectionkey}}{{$key}}">
                                    ...
                                    <div class="question_details p-2 py-5 shadow">
                                        ...
                                        @elseif($question['questionType'] == 'MTF')
                                            @foreach($question['options']['matches'] as $optionKey => $option)
                                                <ul class="question_options_container m-0 p-0">
                                                    ...
                                                    <li style="max-width: 50%;" class="question_options_items p-2 text-decoration-none text-dark rounded-3 overflow-hidden border border-2 shadow-sm d-flex justify-content-start align-items-center
                                                        @if(count($question['user_answer']) > 0)
                                            ...
                                            @endforeach
                                            @if(!$question['is_correct'] && count($question['user_answer']) > 0)
                                                <div class="text-center fs-5 mb-3">
                                                        Correct answers are
                                                        @foreach($question['ca'] as $caKey => $ca)
                                        ...
                                        @elseif($question['questionType'] == 'MSA')
                                            ...
                                            @if(!$question['is_correct'])
                                                <div class="text-center fs-5 mb-3">
                                                    Correct answer is 
                                                    <span class="badge text-black bg-label-primary p-1">Option: {{$question['ca']}}.</span> <span class="ms-1 me-2 d-inline-block">{!!$question['options'][--$question['ca']]!!}</span>
                                                </div>
                                            @endif
