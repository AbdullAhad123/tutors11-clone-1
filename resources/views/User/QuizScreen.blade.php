...
<body>
    ...
    <div class="bg-white shadow fixed-top container-fluid pb-2">
        <div class="d-flex align-items-center justify-content-between">
            ...
            <div class="text-center">
                <h4 class="my-2">{{$quiz['title']}}</h4>
                <h6>Question <span id="current_ques">1</span>/{{$totalQuestions}}</h6>
            </div>
            ...
            @foreach($questions as $key => $question)
                ...
                " data-index="{{++$key}}" data-id="{{$question['id']}}" data-type="{{$question['questionType']}}" data-code="{{$question['code']}}">
                    <div class="my-2 d-flex align-items-center justify-content-between h6">
                        <div>
                            <span class="fw-semibold">Q{{$key}} of {{$totalQuestions}}</span>
    ...
    <script>
        let ques_answered = {!! str_replace("'", "\'", json_encode($answeredQuestions)) !!};
        let ques_total = {!! str_replace("'", "\'", json_encode($totalQuestions)) !!};
        ...
        $(document).on('click', '#finish, .finish', function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{route('fetch_quiz_questions', ['quiz' => $quiz['slug'], 'session' => $session['code']])}}",
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function(data) {
                    $.ajax({
                        type: "POST",
                        url: "{{route('finish_quiz_session', ['quiz' => $quiz['slug'], 'session' => $session['code']])}}",
                        data: {
                            questions: data,
                        ...
                        success: function(data) {
                            if(data.success){
                                window.location.href = "{{route('quiz_thank_you', ['quiz' => $quiz['slug'], 'session' => $session['code']])}}"
                            }
                        },
                        error: function(data, textStatus, errorThrown) {
                            console.log(textStatus);
        ...
        function submitMSA(answer){
            $.ajax({
                type: "POST",
                url: "{{route('update_quiz_answer', ['quiz' => $quiz['slug'], 'session' => $session['code']])}}",
                data: {
        ...
        function activeQuesIndex() {
            return $(".question_content.active").data("index");
        }
        function Fullscreen(){
        ...
        function currentQuestionIndex() {
            $("#current_ques").text(activeQuesIndex());
        }
