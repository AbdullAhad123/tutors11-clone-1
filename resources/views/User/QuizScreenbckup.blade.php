...
<head>
    ...
    <!-- Fonts -->
    <!-- Icons. Uncomment required icon fon ts -->
...
<body class="overflow-hidden">
    ...
    <div class="layout-wrapper layout-content-navbar">
        <div class="layout-page" style="height: 98vh;">
            <!-- navbar  -->
            <nav class="bg-white cutomized_container_fluid exam_navbar pb-3 position-fixed px-2 py-2 rounded-top-4 shadow top-0 w-100">
                <div class="d-flex justify-content-between align-items-center h-100 mx-auto mx-lg-5 mx-md-4 mx-sm-2 mx-1">
                    ...
                    <div class="exam_name_side my-2">
                        <h1 class="fw-semibold h4 mt-0">{{$quiz['title']}}</h1>
                        <h2 class="h5 text-center mb-0"><span id="answeredQuestions">{{$answeredQuestions}}</span>/{{$quiz['total_questions']}} Answered</h2>
                    </div>
                    <div class="align-items-center exam_details_side my-2">
                        <div class="d-flex justify-content-end">
                            <span class="countdown_timer mx-1 bg-warning-subtle py-2 px-3 rounded-3" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Timer">
                            ...
                            <span class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <li><a class="dropdown-item px-3 py-3"   data-toggle="modal" data-target="#finishTest"   class="fs-4 text-black py-2"><i class="fs-4 bx bx-log-out"></i> End Session</a></li>
                                </ul>
                            </span>
                        </div>
                    </div>
                </div>
                <div class="progress rounded-3 my-0 mt-2" style="height: 10px;">
                    <div class="progress-bar bg-primary progress-bar-striped" id="progressBar" role="progressbar" style="width:{{round(($answeredQuestions / $quiz['total_questions']) * 100)}}%"></div>
                </div>
            </nav>
            <!-- main content  -->
            <div class="main_content_wrapper">
                ...
                <div class="bg-white content_container overflow-scroll">
                    <div class="pt-4 p-2">
                        <div class="content_question p-1 px-3 mx-auto py-4 pb-5" style="max-width: 1180px;">
                            <div class="tab-content" id="questions_wrap"></div>
                        </div>
                        <div class="bg-white bottom-0 content_question_bottombar_btns position-fixed py-4 shadow-lg w-100 z-3">
                            <div class="content_container_bottom px-2 container">
                                <div class="row">
                ...
                <div class="d-none" data-bs-scroll="true"
                    ...
                    <div class="offcanvas-header py-4">
                        <h6 class="offcanvas-title text-uppercase">{{$quiz['title']}}</h6>
                        ...
                        <div class="row align-items-center">
                            <div class="col-6 px-3">
                                <div class="d-inline-flex text_black tooltip_custom" data-bs-toggle="tooltip"
                                    ...
                                    <span id="answeredQuestions">{{$answeredQuestions}}</span>/{{$quiz['total_questions']}} Answered
                                </div>
                            </div>
                            <div class="col-6">
    ...
    <div class="submit_loading text-white z-5 d-none">
    ...
    <div class="time_alert_container py-4">
        <div class="h-100 w-100 d-flex align-items-center justify-content-center">
            <div id="time_alert" class="align-items-center d-flex fw-normal mx-auto px-4 py-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="me-1" width="24" height="24" viewBox="0 0 24 24" style="fill: #db0000;"><path d="M11.001 10h2v5h-2zM11 16h2v2h-2z"></path><path d="M13.768 4.2C13.42 3.545 12.742 3.138 12 3.138s-1.42.407-1.768 1.063L2.894 18.064a1.986 1.986 0 0 0 .054 1.968A1.984 1.984 0 0 0 4.661 21h14.678c.708 0 1.349-.362 1.714-.968a1.989 1.989 0 0 0 .054-1.968L13.768 4.2zM4.661 19 12 5.137 19.344 19H4.661z"></path></svg>
                <span id="timeAlert_msg"></span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let answeredQuestions = {!! str_replace("'", "\'", json_encode($answeredQuestions)) !!};        
        let total_questions = {!! str_replace("'", "\'", json_encode($quiz['total_questions'])) !!};        
        var time = 1;
        ...
        fetchQuestions("/quiz/{{$quiz['slug']}}/questions/{{$session['code']}}")
        function fetchQuestions(reqUrl) {
            $.ajax({
                type: "GET",
                url: reqUrl,
                data: {
                ...
                success: function(data) {
                    ...
                    questions.forEach(function(question, i) {
                        ...
                        if(i == answeredQuestions){
                            ...
                            question_box += `<div id="`+question.code+`" data-type="`+question.questionType+`" data-status="`+question.status+`" data-index="`+i+`" data-ques="`+question.id+`" class="question_tab tab-pane fade in active"><div>`+ wrong_ques_modal +`</div><div class="animate__animated animate__fadeIn"><div class="mb-4 d-flex align-items-center justify-content-between"><div class="h5"><span class="fw-semibold">Q`+index+` of `+total_questions+`</span> <span class="fw-normal text-uppercase"> | `+question.skill+`</span></div></div><div class="fs-2 mt-4 mb-5 text-black">`+question.question+`</div>`+answer+modal+`</div></div>`;
                        } else {
                            buttons +=  `<li class="question_btn">
                                            <a class="question_btn_a border border-dark border-opacity-25 d-block mb-4 px-3 py-2 rounded text-decoration-none text-secondary" data-toggle="tab" href="#`+question.code+`">`+question.question+`</a>
                                        </li>`;
                                    question_box += `<div id="`+question.code+`" data-type="`+question.questionType+`" data-status="`+question.status+`" data-index="`+i+`" data-ques="`+question.id+`" class="question_tab tab-pane fade"><div>`+ wrong_ques_modal +`</div><div class="animate__animated animate__fadeIn"><div class="mb-4 d-flex align-items-center justify-content-between"><div class="h5"><span class="fw-semibold">Q`+index+` of `+total_questions+`</span> <span class="fw-normal text-uppercase"> | `+question.skill+`</span></div></div><div class="fs-2 mt-4 mb-5 text-black">`+question.question+`</div>`+answer+modal+`</div></div>`;
                        }
                    });
        ...
        // function timeAlert() {
        //     const time = $("#count_downTimer").text();
        //     if (time == '01:00') {
        //         $(".time_alert_container").slideDown();
        //         $("#timeAlert_msg").text('You have only 60 seconds to complete the test!');
        //     }

        //     setTimeout(() => {
        //         $(".time_alert_container").slideUp();
        //     }, 2000);
        // }
                    ...
                    success: function(data) {
                        if(data.success){
                            setTimeout(function() {
                                $("#query_error_"+id).text("");
                                let modal_loader = $(".modal_loader").fadeOut();
                                // let tab = `<div class="modal_header p-2 py-4"><div class="text-center"><svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" style="fill:#fff"><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"></path><path d="M9.999 13.587 7.7 11.292l-1.412 1.416 3.713 3.705 6.706-6.706-1.414-1.414z"></path></svg></div><button class="btn modal_dissmiss_btn bg-transparent border-0 rounded-0 shadow-none" data-bs-dismiss="modal" aria-label="Close"><i class="fa-solid fa-xmark text-white fs-5"></i></button></div><div class="modal_body p-2 py-4"><h2 class="h1 mb-2 my-3 text-center text-uppercase">Great!</h2><p class="fs_md mb-4 mt-3 text-center">Thanks for your feedback! Our team will now review it and address issues promptly.</p></div>`
        ...
        $(document).on('click', '#submit', function(){
            ...
            if(type == "FIB"){
                ...
                submitORD(ORD_answer, 'submit');
            } else if(type == "SAQ"){
                let value = $(".question_tab.active").find(".answer_inp").val();
                submitSAQ(escapeHtmlEntities(value), 'submit');
            }
        }); 
        ...
        $(document).on('click', '#review', function(){
            ...
            if(type == "FIB"){
                ...
                let status = $(".question_tab.active").find(".drop_card .form-control").hasClass("bg-success-subtle")?"answered_mark_for_review":"mark_for_review";
                submitORD(ORD_answer, status);
            } else if(type == "SAQ"){
                let value = $(".question_tab.active").find(".answer_inp").val();
                let status = value==""?"mark_for_review":"answered_mark_for_review";
                submitSAQ(escapeHtmlEntities(value), status);
            }
        });
        // FIB Start
            ...
            for (let i = 0; i < question.options; i++) {                
                ...
                let index = i+1;
                    input +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 animate__animated animate__fadeInRight">
                                <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black `+input_group_text+`">`+index+`</span>
                                <input type="text" class="answer_inp border-0 border-secondary form-control ms-2 shadow-none `+form_control+`" placeholder="Type your answer for blank `+index+`" value="`+FIBAnswer+`">
                            </div>`;
            }
        ...
        function submitFIB(answer, action) {
            ...
            $.ajax({
                type: "POST",
                url: '/update_quiz_answer/'+quizSlug+'/'+sessionCode,
                data: {
                ...
                if(question.status == "answered"){
                    if(question.user_answer == index){
                        input +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 MSA_inp_wrap cursor-pointer">
                                <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black bg-success-subtle">`+index+`</span>
                        ...
                        input +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 MSA_inp_wrap cursor-pointer">
                                <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black">`+index+`</span>
        ...
        function submitMSA(answer, action) {
            let current_question = $(".question_tab.active").data("index");
            let id = $(".question_tab.active").attr("id");
            var status;
            if(action == "submit" || action == "clear"){
        ...
        function submitMMA(answer, action) {
            let current_question = $(".question_tab.active").data("index");
            let id = $(".question_tab.active").attr("id");
            var status;
            if(action == "submit" || action == "clear"){
        ...
        function unansweredMTF(question) {
            ...
            if(question.status == "answered"){
                question.user_answer.forEach(function(pair, i) {
                    ...
                    pairs +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 drop_card animate__animated animate__fadeInRight" ondragend="drop(event)">
                                <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black bg-success-subtle"><span class="MTF_index me-1">`+index+` </span> - `+pair.code+`</span>
                ...
                question.options.pairs.forEach(function(pair, i) {
                    let index = i+1;
                    pairs +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 drop_card" ondragend="drop(event)">
                                    <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black"><span class="MTF_index me-1">`+index+` </span> - `+pair.code+`</span>
            ...
            let tab =  `<div><div class="d-inline-block font-size-14 fw-semibold my-2 notice py-1 rounded text-uppercase">
                        ...
                        <div class="answer_wrap row mb-5 pb-5">
                            ...
                            <div class="col-6 ps-2 drop"><div class="drop_container" id="drop_items_`+question.code+`">`+pairs+`</div></div>
                        </div>`;
            return tab;
        }
        function submitMTF(answer, action) {
            let current_question = $(".question_tab.active").data("index");
            let id = $(".question_tab.active").attr("id");
            var status;
            if(action == "submit" || action == "clear"){
        ...
        function submitTOF(answer, action) {
            let current_question = $(".question_tab.active").data("index");
            let id = $(".question_tab.active").attr("id");
            var status;
            if(action == "submit" || action == "clear"){
            ...
            if(question.status == "answered"){
                question.user_answer.forEach(function(ord, i) {
                    ...
                    order +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 drop_card animate__animated animate__fadeInRight" ondragend="drop(event)">
                                <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black bg-success-subtle"><span class="MTF_index me-1">`+index+` </span> - `+ord.code+`</span>
                ...
                question.options.forEach(function(ord, i) {
                    let index = i+1;
                    order +=  `<div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4 drop_card animate__animated animate__fadeInRight" ondragend="drop(event)">
                                    <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black"><span class="MTF_index me-1">`+index+` </span> - `+ord.code+`</span>
        ...
        function submitORD(answer, action) {
            let current_question = $(".question_tab.active").data("index");
            let id = $(".question_tab.active").attr("id");
            var status;
            if(action == "submit" || action == "clear"){
        ...
        function unansweredSAQ(question) {
            ...
            let tab =  `<div><div class="d-inline-block font-size-14 fw-semibold my-2 notice py-1 rounded text-uppercase">Type your answer in the below text box</div></div>
                        <div class="answer_wrap mb-5 pb-5">
                            <div class="border border-2 border-primary input-group my-2 p-3 py-2 rounded-4">
                                <span class="border-0 input-group-addon p-0 px-3 rounded-3 small text-black `+input_group_text+`">1</span>
        ...
        function submitSAQ(answer, action) {
            let current_question = $(".question_tab.active").data("index");
            let id = $(".question_tab.active").attr("id");
            var status;
            if(action == "submit" || action == "clear"){
        ...
        $(document).on('click', '#finish', function(){
            $.ajax({
                type: "GET",
                url: "/quiz/"+quizSlug+"/questions/"+sessionCode,
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function(data) {
                    $.ajax({
                        type: "POST",
                        url: "/quiz/"+quizSlug+"/finish/"+sessionCode,
                        data: {
                            questions: data,
                            total_time_taken: totalTimeTaken,
                            _token: '{{ csrf_token() }}'
                        ...
                        success: function(data) {
                            if(data.success){
                                window.location.href = "/quiz/"+data.quiz+"/thank-you/"+data.session;
                            }
                        },
                        error: function(data, textStatus, errorThrown) {
                            console.log(textStatus);
        ...
        // });
        $(document).on("click", "#report_issue", function(event) {
