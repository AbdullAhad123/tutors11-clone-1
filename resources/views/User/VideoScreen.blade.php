...
<head>
    ...
    <title>{{$skill['name']}} Videos - Student Portal - Tutors 11 Plus</title>
...
<body>
    ...
    <div id="main" class="bg-light overflow-scroll p-0 position-relative vh-100">
        <div class="bg-white py-3 px-4 shadow-sm position-sticky w-100 top-0">
            <div class="d-flex m-auto justify-content-between" style="max-width: 900px;">
                ...
                <h5 class="fw-normal mb-0 align-self-center px-2">Video <span id="active_video_num"></span>: <span
                        id="active_video_name"></span></h5>
                <a href="/videos/{{$category['slug']}}/{{$section['slug']}}/{{$skill['slug']}}"
    ...
    <script>
        ...
        fetchPracticeVideos('/fetch-practice-videos/' + categorySlug + '/' + sectionSlug + '/' + skillSlug + '?page=' + page + '&withBody=1', 'default');
        function fetchPracticeVideos(reqUrl, move_page) {
            $.ajax({
                type: "GET",
                url: reqUrl,
                data: {
                ...
                success: function (data) {
                    if (data) {
                        ...
                        videos.forEach(function (video, i) {
                            let index = ++i;
                            i = --i;
                            let button = `<button class="d-flex align-items-center mb-3 w-100 bg-light border-0 rounded-1 py-2 video_btn" data-page="` + pagination.current_page + `" id="video-tab-` + page + i + `" data-bs-toggle="tab" data-bs-target="#video-` + page + i + `" type="button" role="tab" aria-controls="video-` + page + i + `" aria-selected="false">
                                                <div id="serial-`+ page + i + `" class="badge me-2 fw-normal" style="background-color: #333;">` + index + `</div>
                                                <div>
                                                    <div class="video_title text-start">`+ video.title + `</div>
                                                    <div class="align-items-center d-flex fw-light lh-sm text-start" style="font-size: 13px;"> 
                                                        ...
                                                        `+ video.watch_time + ` Min Read
                                                    </div>
                                                </div>
                                            </button>`;
                            let tab = `<div class="bg-white shadow-sm rounded py-4 px-5 tab-pane fade video_tab" id="video-` + page + i + `" role="tabpanel" aria-labelledby="video-tab-` + page + i + `">
                                            <div id="`+ video.code + `"></div>
                                            <h5>`+ video.title + `</h5>
                                            <div id="description`+ page + i + `"></div>
                                        </div>`;
                            $("#nav-tab").append(button);
                            $("#nav-tabContent").append(tab);
                            if(video.subscription){
                                if (video.type == 'youtube') {
                                    $("#" + video.code).append('<iframe class="w-100 mb-3" style="height: 500px;" src="https://www.youtube.com/embed/' + video.link + '" frameborder="0" style=""></iframe>');
                                } else {
                                    $("#" + video.code).append('<iframe id="videoPreview" class="w-100 mb-3" style="height: 500px;" src="https://player.vimeo.com/video/' + video.link + '" frameborder="0" style=""></iframe>');
                                }
                            } else {
                                $("#" + video.code).append('<div class="alert alert-danger mt-4 small">You don\'t have an active plan to access this content. Please subscribe.</div>');
                            }
                            if (video.description) {
                                $("#description" + page + i).append(video.description).addClass("fw-light pt-1 text-muted");
                            } else {
                                $("#description" + page + i).append('No description found').addClass("fw-light pt-1 text-muted");
                            }
                            if (i == start) {
                                $("#video-tab-" + page + i).addClass("active").attr("aria-selected", "true");
                                $("#video-" + page + i).addClass("show active");
                                $("#active_video_name").text(video.title);
        ...
        $(document).on('click', '#next', function () {
            let hasNext = $(".video_btn.active").next().length;
            if (hasNext > 0) {
                $(".video_btn.active").removeClass("active").next().addClass("active");
                $(".video_tab.active").removeClass("active show").next().addClass("active show");
                $("#active_video_num").text(parseInt($("#active_video_num").text()) + 1);
                let video_title = $(".video_btn.active").find(".video_title").text();
                $("#active_video_name").text(video_title);
            }
        });
        $(document).on('click', '#prev', function () {
            let hasPrev = $(".video_btn.active").prev().length;
            if (hasPrev > 0) {
                $(".video_btn.active").removeClass("active").prev().addClass("active");
                $(".video_tab.active").removeClass("active show").prev().addClass("active show");
                $("#active_video_num").text(parseInt($("#active_video_num").text()) - 1);
                let video_title = $(".video_btn.active").find(".video_title").text();
                $("#active_video_name").text(video_title);
            }
        ...
        $(".move_page").click(function () {
            ...
            if (url && url != "") {
                fetchPracticeVideos(url + '&withBody=1', 'move_page');
            }
        });
        $(document).on('click', '.video_btn', function () {
            $("#active_video_num").text($(this).find(".badge").text());
            $("#active_video_name").text($(this).find(".video_title").text());
        });
